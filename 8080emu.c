...
int main (int argc, char **argv)
{
    ...
    /* fseek sets the file position of the stream to the given offset (0L in this case)*/
    fseek(f, 0L, SEEK_END);
    int fsize = ftell(f);
    fseek(f, 0L, SEEK_SET);

    // allocate memory for the buffer to be the size of the file
...
int Disassemble8080p(unsigned char *codebuffer, int pc)
{
    ...
    switch (*code)
    {
        ...
        case 0x45: printf("MOV  B, H"); break;
        ...
        case 0xdc: printf("OUT  #$%02x", code[1]); opbytes = 2; break;
    ...
    // Forgot this intially :/ now we know how far to traverse to the next byte / address bit 
    return opbytes;
}